...
## Purpose
Defines the Deno runtime requirement for the container image, ensuring the Deno binary is installed from the official multi-arch Docker image (`denoland/deno:bin`) into `/usr/bin/deno` with appropriate ownership and permissions (`$UID:0`, mode `775`) for non-root execution.
