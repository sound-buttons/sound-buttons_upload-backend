# kubernetes-deployment-security Specification

## Purpose
Defines the Kubernetes pod-level security posture the `sound-buttons_upload-backend` Helm chart must enforce for the backend workload. It currently governs ServiceAccount API-token automounting: because the backend never calls the Kubernetes API server, the chart must keep the unused ServiceAccount token out of the pod by default (defense-in-depth) while remaining configurable for operators who genuinely need in-cluster API access.
## Requirements
