# container-init-process Specification

## Purpose
Defines the container's PID-1 init contract: a pinned, integrity-verified
`dumb-init` runs as process ID 1 to forward termination signals to the Azure
Functions host and reap zombie child processes (`yt-dlp`, `ffmpeg`,
`bgutil-pot`). It governs how the init binary is sourced — from the official
upstream release with SHA256 verification, decoupled from unrelated tool images —
so the security-critical PID-1 binary's version and integrity are explicit and
independent of other image tooling.
## Requirements
